function [Xsol]=CCKmeansBalancedOutliers_SDP_Yalmip(D,Ck)
...
Y=(M+ones(N,1)*ones(N,1)'+x*ones(N,1)'+ones(N,1)*x');
% Define an objective
D=D.^2;
Objective = sum(sum(D.*Y));

Constraints = [K*x+x0==(1-K)*ones(N,1)];
...
Constraints=[Constraints, sum(x0)==2*n0-N,M0*ones(N,1)==(2*n0-N)*x0,diag(M0)==1,[1,x0';x0,M0]>=0];
...
%options = sdpsettings('verbose',1,'solver','sdpnal','sdpnal.maxiter',1500);
